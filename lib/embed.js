...
// model probe, which is philotas's own request and which the client has no
...
const DEFAULT_APP = 'philotas/embed';
...
async function servedModel(opts) {
  ...
  // process, and the embedder would come back while philotas did not.
  //
  // KNOWN LIMITATION, stated plainly rather than papered over: an in-place model
  // swap under a RUNNING philotas process is not noticed until restart. Vectors
