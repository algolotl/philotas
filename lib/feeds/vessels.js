// Vessels — every hull in the picture, from three real sources.
//
//   1. Ferry positions, live, from Transport for NSW. Real tracks.
//   2. Commercial shipping from Port Authority of NSW: cargo, container,
//      tanker, cruise. Berth assignments rather than positions, so these are
//      placed AT their berth and marked `position_source: 'berth'`.
//   3. Position history written by parallax-ingest, so the map keeps working on
...
export async function fetchVessels(region, options = {}) {
  ...
  // 2. Sydney Ferries, live from Transport for NSW. Genuinely real vessels,
  ...
  // 3. Commercial shipping from Port Authority of NSW. Used with permission.
  ...
  // Name only the sources that actually survived deduplication, so the label
  // cannot claim a feed that contributed nothing visible.
  const surviving = new Set(vessels.map((v) => v.feed_source).filter(Boolean));
  const source = surviving.size ? sources.filter((s) => surviving.has(s)).join('+') : 'none';

  const features = vessels
    .filter((vessel) => Array.isArray(vessel.position) && inBbox(vessel.position, region?.bbox))
    .map((vessel) => toFeature({ ...vessel }, { source: vessel.feed_source || source, now }));
  ...
  const emptyIsOutage = vessels.length === 0 && liveSourcesFailed;
  ...
  return {
    ...
    notice: vessels.length === 0 ? emptyReason() : null,
  };
}
...
function emptyReason() {
  if (!process.env.TFNSW_API_KEY) {
    return 'No vessels — Transport for NSW key not configured, so ferry positions are unavailable.';
  }
  return 'No vessels currently reported by either the ferry feed or the port movement schedule.';
}
