...
//   - a local JSON file otherwise (.data/parallax-db.json — zero-config dev)
...
// --- maritime: written by parallax-ingest, read-only from the app ---
...
function fileBackend() {
  ...
  const FILE = path.join(DIR, 'parallax-db.json');
  let db = null;
...
export const BASE_SCHEMA_STATEMENTS = [
  ...
  // Maritime. Written by parallax-ingest, read by the app.
...
function pgBackend() {
  ...
  async function init() {
    ...
    // `DATABASE_URL=postgres://unused:unused@127.0.0.1:1/parallax-must-not-connect
