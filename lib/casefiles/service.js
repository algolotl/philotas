...
//
// Two sources feed it:
//   - detector events written by philotas-ingest (AIS: gaps, loitering, ...)
...
export async function getCaseFiles(region) {
  ...
  // An assessment already running for this region is the answer too. The call it
  // waits on takes up to 25s, and until it resolves the check above still misses
  // — so without this, every poll during that window started its own model call.
  // On the trial that stampede was the load behind both the 19,813 fallback
  // warnings and the heap that hit its limit. lib/cache.js solves the same
  // problem the same way for feeds.
  if (hit && hit.pending) return hit.pending;

  const pending = computeCaseFiles(region).then(
    (files) => {
      cache.set(region.id, { at: Date.now(), files });
      return files;
    },
    (err) => {
      // A failure is not cached: the next caller should be free to try again.
      cache.delete(region.id);
      throw err;
    },
  );
  cache.set(region.id, { at: now, pending });
  return pending;
}

async function computeCaseFiles(region) {
  const now = Date.now();
  const candidates = [];
  ...
  return assessCaseFile(files, region);
}
