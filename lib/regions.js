// Region definitions — the thing that makes Parallax "drop in a new city".
//
...
export function place({ id, name, type = 'city', country, theatre, center, half = [0.6, 0.5], newsHalf, zoom = 9.5, observer = true, weatherUrl, sites = [], adsbCoverage }) {
  ...
  // covers the world, unlike the transport, vessel and camera layers which
  // each need a national integration. It reads zero over most cities most of
  // the time and lights up during a fire season, which is honest either way.
  const layers = ['aviation', 'satellites', 'seismic', 'hotspots', 'news', 'events'];
  const params = {
...
export const REGIONS = {
  ...
  // ---- market regions ----
  //
  // Added 2026-08-15 for the markets Parallax is being sold into: New Zealand,
