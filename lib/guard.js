...
// Set PHILOTAS_OPEN_READ=1 to drop the read gate entirely. Intended for a
...
function openRead() {
  const on = process.env.PHILOTAS_OPEN_READ === '1';
  if (on && !openReadWarned) {
    openReadWarned = true;
    console.warn('[guard] PHILOTAS_OPEN_READ=1 — read routes are unauthenticated');
  }
  return on;
}
